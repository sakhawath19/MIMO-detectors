...
intermediate_sig = tf.placeholder(tf.float32, shape=[None, user], name='input')
...
'''...............'''
h_fc_2 = []
W_fc_input_2 = weight_variable([user, fc_size])
b_fc_input_2 = bias_variable([fc_size])
h_fc_2.append(tf.nn.relu(tf.matmul(intermediate_sig, W_fc_input_2) + b_fc_input_2))
...
'''........................'''
ssd_2 = tf.reduce_sum(tf.square(transmitted_sig - h_final_2))

global_step_2 = tf.Variable(0, trainable=False)
learning_rate_2 = tf.train.exponential_decay(startingLearningRate, global_step, decay_step_size, decay_factor,
                                           staircase=True)
train_step_2 = tf.train.AdamOptimizer(learning_rate_2).minimize(ssd_2)
'''........................'''
...
for i in range(train_iter):
    ...
    if i % 100 == 0:
        ...
        print('Error_', eq3)
        if correct_bits > 19850:
            break
    train_step.run(feed_dict={received_sig: batch_HY, transmitted_sig: batch_X, batchSize: batch_size})
    val_3, final_3 = sess.run([val_2, final_2],
                              feed_dict={received_sig: batch_HY, transmitted_sig: batch_X, batchSize: batch_size})

    if i % 100 == 0:
        val_4, final_4 = sess.run([val_2, final_2],
                              feed_dict={received_sig: batch_HY, transmitted_sig: batch_X, batchSize: batch_size})
        eq4 = error_rate(val_4, final_4)
        print('Error in second net_', eq4)

    false_detection = generate_intermediate_input(val_3, final_3)
    train_step_2.run(feed_dict={intermediate_sig: false_detection, transmitted_sig: batch_X, batchSize: batch_size})
    #print('here')
