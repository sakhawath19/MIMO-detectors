...
def error_rate(transmitted_symbol, estimated_symbol):
    ...
    v2 = np.matmul(np.ones([K * B, 1]), CONS_ALPHABET)
    ...
    error_zf = 1 - (np.sum(accuracy_zf) / (K * B))
...
NNinput = tf.placeholder(tf.float32, shape=[None, K], name='input')
...
# The network
h_fc = []
W_fc_input = weight_variable([K, fc_size])
...
h_final = tf.matmul(h_fc[num_of_hidden_layers-1], W_fc_final) + b_final
...
val_2 = tf.reshape(org_siganl, [K * B, 1])
final_2 = tf.reshape(h_final, [K * B, 1])
...
for i in range(train_iter):
    ...
    if i % 100 == 0:
        correct_bits = eq2.eval(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
        train_accuracy = accuracy.eval(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
        val_3, final_3 = sess.run([val_2, final_2],feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
        eq3 = error_rate(val_3, final_3)
        print("step %d, loss is %g, number of correct bits %d" % (i, train_accuracy, correct_bits))
        print('Error_', eq3)
    train_step.run(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
...
for i_snr in range(num_snr):
    ...
    for i in range(test_iter):
        ...
        tic = tm.time()
        tmp_bers[0][i] = eq2.eval(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
        val_3, final_3 = sess.run([val_2, final_2], feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
        tmp_bers_2[0][i] = error_rate(val_3, final_3)
        toc = tm.time()
        tmp_times[0][i] = toc - tic
        if i % 100 == 0:
            eq2.eval(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
            train_accuracy = accuracy.eval(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B})
            print("test accuracy %g" % eq2.eval(feed_dict={NNinput: batch_HY, org_siganl: batch_X, batchSize: B}))
...
print(bers)
print('Bit error rates from new method:')
print(bers_2)
