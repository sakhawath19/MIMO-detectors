...
batch_size =1000
sample_per_values = 4
sample_size = sample_per_values * num_values
N = sample_size
K = num_values
B = batch_size
#low_snr = 0.7
...
num_snr = 6
hidden_nodes_layers = 20
hidden_nodes_output_layer = 80
...
'''
def network(y):
    c1, c2, c3, c4 = tf.split(y, num_values, 1)
...
s1, s2, s3, s4 = tf.split(NNinput, num_values, 1)

h_fc_1 = []
W_fc_input = weight_variable([4, hidden_nodes_layers])
b_fc_input = bias_variable([hidden_nodes_layers])

h_fc_1.append(tf.nn.relu(tf.matmul(s1, W_fc_input) + b_fc_input))

for i in range(num_of_hidden_layers):
    h_fc_1.append(relu_layer(h_fc_1[i - 1], hidden_nodes_layers, hidden_nodes_layers, 'relu' + str(i)))

W_fc_final = weight_variable([hidden_nodes_layers, sample_per_values])
b_final_1 = bias_variable([sample_per_values])

h_final_1 = tf.matmul(h_fc_1[-1], W_fc_final) + b_final_1

''' parallel network 2 '''
h_fc_2 = []

h_fc_2.append(tf.nn.relu(tf.matmul(s1, W_fc_input) + b_fc_input))

for i in range(num_of_hidden_layers):
    h_fc_2.append(relu_layer(h_fc_2[i - 1], hidden_nodes_layers, hidden_nodes_layers, 'relu' + str(i)))

W_fc_final = weight_variable([hidden_nodes_layers, sample_per_values])
b_final_2 = bias_variable([sample_per_values])

h_final_2 = tf.matmul(h_fc_2[-1], W_fc_final) + b_final_2

''' parallel network 3 '''
h_fc_3 = []

h_fc_3.append(tf.nn.relu(tf.matmul(s1, W_fc_input) + b_fc_input))

for i in range(num_of_hidden_layers):
    h_fc_3.append(relu_layer(h_fc_3[i - 1], hidden_nodes_layers, hidden_nodes_layers, 'relu' + str(i)))

W_fc_final = weight_variable([hidden_nodes_layers, sample_per_values])
b_final_3 = bias_variable([sample_per_values])

h_final_3 = tf.matmul(h_fc_3[-1], W_fc_final) + b_final_3

''' parallel network 4 '''
h_fc_4 = []
h_fc_4.append(tf.nn.relu(tf.matmul(s1, W_fc_input) + b_fc_input))

for i in range(num_of_hidden_layers):
    h_fc_4.append(relu_layer(h_fc_4[i - 1], hidden_nodes_layers, hidden_nodes_layers, 'relu' + str(i)))

W_fc_final = weight_variable([hidden_nodes_layers, sample_per_values])
b_final_4 = bias_variable([sample_per_values])

h_final_4 = tf.matmul(h_fc_4[-1], W_fc_final) + b_final_4

con_1_2 = tf.concat([h_final_1,h_final_2],1)
con_3 = tf.concat([con_1_2,h_final_3],1)
con_4 = tf.concat([con_3, h_final_4],1)

''' here 4 is the size of con_4'''
W_fc_final_all = weight_variable([4 * 4, K])
b_final_all = bias_variable([K])

h_final = tf.matmul(con_4, W_fc_final_all) + b_final_all
