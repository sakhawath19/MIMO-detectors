...
H = np.random.randn(antenna, user)
...
def least_square_error_rate(transmitted_symbol, estimated_symbol):
    ...
    false_detection = user * batch_size - total_detected_symbol
    error_ls = false_detection / (user * batch_size * Num_of_symbol_bits)
...
# bers = 1 - bers / (user * batch_size * Num_of_symbol_bits)
# print('Bit error rates from previous method:', bers)

print('Bit error rates calculated using least square error:', bers_2)

fig1 = plt.figure('16QAM, TxR:20x30, Iteration:6000, SNR:7-12, Batch size:1000')
ax1 = fig1.add_subplot(111)
# ax1.plot(snr_list_db.reshape(-1), bers.reshape(-1), color='black', marker='*', linestyle='-', linewidth=1, markersize=6, label='FC')
ax1.plot(snr_list_db.reshape(-1), bers_2.reshape(-1), color='blue', marker='d', linestyle='-', linewidth=1, markersize=6, label='NN')
